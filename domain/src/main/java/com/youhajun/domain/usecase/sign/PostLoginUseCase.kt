...
) : UseCase<LoginRequestVo, Flow<UiState<Unit>>>() {

    @OptIn(ExperimentalCoroutinesApi::class)
    override suspend fun invoke(request: LoginRequestVo): Flow<UiState<Unit>> {
        return signRepository.postLogin(LoginRequestVo.mapModelToDto(request)).flatMapConcat {
            when (it) {
